...
cdef extern from "tdnac.c":
    void TD_NAC_s(int istep, int nst, int nbasis, int norb, int *nocc, int *nvirt, double dt, double **nacme, double **ao_overlap, double ***mo_coef_old, double ***mo_coef_new, double ***ci_coef_old_u, double ***ci_coef_old_d, double ***ci_coef_new_u, double ***ci_coef_new_d)
...
def wf_overlap_s(theory, molecule, istep_py, dt_py):
    cdef:
        double **nacme
        double **ao_overlap
        double ***mo_coef_old
        double ***mo_coef_new
        double ***ci_coef_old_u
        double ***ci_coef_old_d
        double ***ci_coef_new_u
        double ***ci_coef_new_d
        double dt
        int istep, ist, nst, ibasis, jbasis, iorb, jorb, nbasis, norb, *nocc, *nvirt, spi
    ...
    nocc = <int*> PyMem_Malloc(2 * sizeof(int))
    nvirt = <int*> PyMem_Malloc(2 * sizeof(int))

    for spi in range(2):
        nocc[spi] = theory.nocc[spi]
        nvirt[spi] = theory.nvirt[spi]
    ...
    for iorb in range(norb):
        for jorb in range(nbasis):
            mo_coef_old[iorb][jorb] = <double*> PyMem_Malloc(2 * sizeof(double))
            mo_coef_new[iorb][jorb] = <double*> PyMem_Malloc(2 * sizeof(double))

    ci_coef_old_u = <double***> PyMem_Malloc(nst * sizeof(double**))
    ci_coef_old_d = <double***> PyMem_Malloc(nst * sizeof(double**))
    ci_coef_new_u = <double***> PyMem_Malloc(nst * sizeof(double**))
    ...
    for ist in range(nst):
        for iorb in range(nocc[0]):
            for jorb in range(nvirt[0]):
                ci_coef_old_u[ist][iorb][jorb] = theory.ci_coef_old_u[ist, iorb, jorb]
                ci_coef_new_u[ist][iorb][jorb] = theory.ci_coef_new_u[ist, iorb, jorb]
        for iorb in range(nocc[1]):
            for jorb in range(nvirt[1]):
                ci_coef_old_d[ist][iorb][jorb] = theory.ci_coef_old_d[ist, iorb, jorb]
                ci_coef_new_d[ist][iorb][jorb] = theory.ci_coef_new_d[ist, iorb, jorb]

    # Calculate TDNAC term for CIoverlap
    TD_NAC_s(istep, nst, nbasis, norb, nocc, nvirt, dt, nacme, ao_overlap, mo_coef_old, mo_coef_new, ci_coef_old_u, ci_coef_old_d, ci_coef_new_u, ci_coef_new_d)
    ...
    for iorb in range(norb):
        for ibasis in range(nbasis):
            for spi in range(2):
                theory.mo_coef_old[iorb, ibasis, spi] = mo_coef_new[iorb][ibasis][spi]

    for ist in range(nst):
        for iorb in range(nocc[0]):
            for jorb in range(nvirt[0]):
                theory.ci_coef_old_u[ist, iorb, jorb] = ci_coef_new_u[ist][iorb][jorb]
        for iorb in range(nocc[1]):
            for jorb in range(nvirt[1]):
                theory.ci_coef_old_d[ist, iorb, jorb] = ci_coef_new_d[ist][iorb][jorb]
    ...
    for iorb in range(norb):
        for jorb in range(nbasis):
            PyMem_Free(mo_coef_old[iorb][jorb])
            PyMem_Free(mo_coef_new[iorb][jorb])
    ...
    for ist in range(nst):
        for iorb in range(nocc[0]):
            PyMem_Free(ci_coef_old_u[ist][iorb])
            PyMem_Free(ci_coef_new_u[ist][iorb])
        for iorb in range(nocc[1]):
            PyMem_Free(ci_coef_old_d[ist][iorb])
            PyMem_Free(ci_coef_new_d[ist][iorb])

    for ist in range(nst):
        PyMem_Free(ci_coef_old_u[ist])
        PyMem_Free(ci_coef_old_d[ist])
        PyMem_Free(ci_coef_new_u[ist])
        PyMem_Free(ci_coef_new_d[ist])

    PyMem_Free(ci_coef_old_u)
    PyMem_Free(ci_coef_old_d)
    PyMem_Free(ci_coef_new_u)
    PyMem_Free(ci_coef_new_d)
