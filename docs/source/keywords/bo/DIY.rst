...
First, you should define a class for the quantum chemistry program that inherits the BO_calculator. (``program_name.py``)

.. literalinclude:: ../../../../work/src/bo/diy/program_name.py

Then you define classes for theories (ex. DFT, HF, CASSCF, ...) that inherits program class. (``theory1.py``)

.. literalinclude:: ../../../../work/src/bo/diy/theory1.py

In theory class, variables ``molecule.l_nacme`` and ``self.re_calc`` must be initialized. 
...
Theory class must have ``get_bo`` method because ``run`` method in dynamics object calls ``get_bo`` method. The method should get all arguments shown in the example code.
